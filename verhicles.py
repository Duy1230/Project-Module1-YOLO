...
PROCESS = "Processing..."
model = YOLO("model\model_verhicles.pt")

st.title("YOLOv10 for verhicle detection! 🚗")
...
if uploaded_file is not None:
    ...
    image = Image.open("demo\\verhicle.jpg")
    st.image(image)
    with st.spinner(PROCESS):
        process_image('demo\\verhicle.jpg', model)

elif button_video:

    st.video("demo\\verhicle.mp4")
    with st.spinner(PROCESS):
        process_video("demo\\verhicle.mp4", model)
